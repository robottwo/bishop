...
func TestEvaluateCommandHandler(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "non-bish_evaluate command should call next handler",
		...
		},
		{
			...
			args:           []string{"bish_evaluate", "--help"},
			shouldCallNext: false,
			expectError:    false,
		},
		{
			name:           "help short flag should print help",
			args:           []string{"bish_evaluate", "-h"},
			shouldCallNext: false,
			expectError:    false,
		},
		{
			name:           "default values when no args provided",
			args:           []string{"bish_evaluate", "--limit", "3"},
			shouldCallNext: false,
			expectError:    false,
		},
		{
			name:           "custom limit",
			args:           []string{"bish_evaluate", "--limit", "2"},
			shouldCallNext: false,
			expectError:    false,
		},
		{
			name:           "custom limit short flag",
			args:           []string{"bish_evaluate", "-l", "2"},
			shouldCallNext: false,
			expectError:    false,
		},
		{
			name:           "custom model",
			args:           []string{"bish_evaluate", "--limit", "3", "--model", "gpt-4"},
			shouldCallNext: false,
			expectError:    false,
		},
		{
			name:           "custom model short flag",
			args:           []string{"bish_evaluate", "-l", "3", "-m", "gpt-4"},
			shouldCallNext: false,
			expectError:    false,
		},
		{
			name:           "custom iterations",
			args:           []string{"bish_evaluate", "-l", "3", "--iterations", "2"},
			shouldCallNext: false,
			expectError:    false,
		},
		{
			name:           "custom iterations short flag",
			args:           []string{"bish_evaluate", "-l", "3", "-i", "2"},
			shouldCallNext: false,
			expectError:    false,
		},
		{
			name:           "both custom limit and model",
			args:           []string{"bish_evaluate", "-l", "2", "-m", "gpt-4"},
			shouldCallNext: false,
			expectError:    false,
		},
		{
			name:           "error when limit exceeds available entries",
			args:           []string{"bish_evaluate", "--limit", "10"},
			shouldCallNext: false,
			expectError:    true,
		},
		{
			name:           "error when limit exceeds available entries with custom model",
			args:           []string{"bish_evaluate", "--limit", "10", "--model", "gpt-4"},
			shouldCallNext: false,
			expectError:    true,
		},
	}

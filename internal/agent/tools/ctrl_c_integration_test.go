...
func TestUserConfirmationStatelessBehavior(t *testing.T) {
	...
	t.Run("userConfirmation calls should be independent", func(t *testing.T) {
		...
		result1 := userConfirmation(logger, nil, "Test question 1", "Test explanation 1", true)

		// Second call returns y - should not be affected by first call
		userConfirmation = func(logger *zap.Logger, runner *interp.Runner, question string, explanation string, showManage bool) string {
			return "y"
		}
		result2 := userConfirmation(logger, nil, "Test question 2", "Test explanation 2", true)
